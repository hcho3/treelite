...
struct NodeDraft {
  enum class Status : int8_t { kEmpty, kNumericalTest, kCategoricalTest, kLeaf };
  /*
...
void SetLeafVector(
    Tree<ThresholdType, LeafOutputType>* tree, int nid, std::vector<Value> const& leaf_vector) {
  ...
  for (size_t i = 0; i < leaf_vector_size; ++i) {
    Value const& leaf_value = leaf_vector[i];
    TREELITE_CHECK(leaf_value.GetValueType() == expected_leaf_type)
        << "Leaf value at index " << i
        << " has incorrect type. Expected: " << TypeInfoToString(expected_leaf_type)
...
template <typename T>
Value Value::Create(T init_value) {
  Value value;
...
class CreateHandle {
 public:
  inline static std::shared_ptr<void> Dispatch(void const* init_value) {
    auto const* v_ptr = static_cast<ValueType const*>(init_value);
...
Value Value::Create(void const* init_value, TypeInfo type) {
  Value value;
...
T const& Value::Get() const {
  TREELITE_CHECK(handle_);
  T const* out = static_cast<T const*>(handle_.get());
  TREELITE_CHECK(out);
  return *out;
}

TypeInfo Value::GetValueType() const {
  return type_;
}
...
void TreeBuilder::CreateNode(int node_key) {
...
void TreeBuilder::DeleteNode(int node_key) {
...
void TreeBuilder::SetRootNode(int node_key) {
  auto& tree = pimpl_->tree;
  auto& nodes = tree.nodes;
...
void TreeBuilder::SetNumericalTestNode(int node_key, unsigned feature_id, char const* opname,
    Value threshold, bool default_left, int left_child_key, int right_child_key) {
  Operator op = LookupOperatorByName(opname);
  SetNumericalTestNode(node_key, feature_id, op, std::move(threshold), default_left, left_child_key,
      right_child_key);
}

void TreeBuilder::SetNumericalTestNode(int node_key, unsigned feature_id, Operator op,
    Value threshold, bool default_left, int left_child_key, int right_child_key) {
  auto& tree = pimpl_->tree;
  auto& nodes = tree.nodes;
...
void TreeBuilder::SetCategoricalTestNode(int node_key, unsigned feature_id,
...
void TreeBuilder::SetLeafNode(int node_key, Value leaf_value) {
  auto& tree = pimpl_->tree;
  auto& nodes = tree.nodes;
...
void TreeBuilder::SetLeafVectorNode(int node_key, std::vector<Value> const& leaf_vector) {
  auto& tree = pimpl_->tree;
  auto& nodes = tree.nodes;
...
ModelBuilder::ModelBuilder(int num_feature, int num_class, bool average_tree_output,
    TypeInfo threshold_type, TypeInfo leaf_output_type)
    : pimpl_(new ModelBuilderImpl(
        num_feature, num_class, average_tree_output, threshold_type, leaf_output_type)) {}
ModelBuilder::~ModelBuilder() = default;

void ModelBuilder::SetModelParam(char const* name, char const* value) {
  pimpl_->cfg.emplace_back(name, value);
}

int ModelBuilder::InsertTree(TreeBuilder* tree_builder, int index) {
  ...
  if (tree_builder->pimpl_->tree.threshold_type != this->pimpl_->threshold_type) {
    TREELITE_LOG(FATAL) << "InsertTree: cannot insert the tree into the ensemble, because the "
                           "ensemble requires all "
  ...
  for (auto const& kv : tree_builder->pimpl_->tree.nodes) {
    ...
    if (status == NodeDraft::Status::kNumericalTest
      ...
      int const fid = static_cast<int>(kv.second->feature_id);
      if (fid < 0 || fid >= this->pimpl_->num_feature) {
        TREELITE_LOG(FATAL) << "InsertTree: tree has an invalid split at node " << kv.first
                            << ": feature id " << kv.second->feature_id << " is out of bound";
        return -1;
      }
    }
  }
...
TreeBuilder* ModelBuilder::GetTree(int index) {
  return &pimpl_->trees.at(index);
}

TreeBuilder const* ModelBuilder::GetTree(int index) const {
  return &pimpl_->trees.at(index);
}

void ModelBuilder::DeleteTree(int index) {
  auto& trees = pimpl_->trees;
