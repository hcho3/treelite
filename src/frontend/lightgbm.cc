...
template <>
inline int8_t TextToNumber(std::string const& str) {
  errno = 0;
  char* endptr;
  auto val = std::strtol(str.c_str(), &endptr, 10);
...
template <>
inline uint32_t TextToNumber(std::string const& str) {
  errno = 0;
  char* endptr;
...
template <>
inline uint64_t TextToNumber(std::string const& str) {
  errno = 0;
  char* endptr;
...
template <typename T>
inline std::vector<T> TextToArray(std::string const& text, int num_entry) {
...
enum Masks : uint8_t { kCategoricalMask = 1, kDefaultLeftMask = 2 };

enum class MissingType : uint8_t { kNone, kZero, kNaN };
...
inline std::vector<uint32_t> BitsetToList(uint32_t const* bits, size_t nslots) {
...
inline std::unique_ptr<treelite::Model> ParseStream(std::istream& fi) {
  ...
  for (auto const& dict : tree_dict) {
    ...
    if (tree.num_cat > 0) {
      ...
      tree.cat_boundaries = TextToArray<uint64_t>(it->second, tree.num_cat + 1);
  ...
  if (obj_name_ == "multiclass") {
    ...
    std::strncpy(
        model->param.pred_transform, "multiclass_ova", sizeof(model->param.pred_transform));
    ...
    std::strncpy(
        model->param.pred_transform, "logarithm_one_plus_exp", sizeof(model->param.pred_transform));
    ...
    if (sqrt) {
      std::strncpy(
          model->param.pred_transform, "signed_square", sizeof(model->param.pred_transform));
    } else {
  ...
  for (auto const& lgb_tree : lgb_trees_) {
    ...
    while (!Q.empty()) {
      ...
      Q.pop();
      if (old_id < 0) {  // leaf
        double const leaf_value = lgb_tree.leaf_value[~old_id];
        tree.SetLeaf(new_id, static_cast<double>(leaf_value));
        if (!lgb_tree.leaf_count.empty()) {
          int const data_count = lgb_tree.leaf_count[~old_id];
          TREELITE_CHECK_GE(data_count, 0);
          tree.SetDataCount(new_id, static_cast<size_t>(data_count));
        }
      } else {  // non-leaf
        auto const split_index = static_cast<unsigned>(lgb_tree.split_feature[old_id]);
        auto const missing_type = GetMissingType(lgb_tree.decision_type[old_id]);

        tree.AddChilds(new_id);
        if (GetDecisionType(lgb_tree.decision_type[old_id], kCategoricalMask)) {
          // categorical split
          int const cat_idx = static_cast<int>(lgb_tree.threshold[old_id]);
          ...
          // numerical split
          auto const threshold = static_cast<double>(lgb_tree.threshold[old_id]);
          bool default_left = GetDecisionType(lgb_tree.decision_type[old_id], kDefaultLeftMask);
        ...
        if (!lgb_tree.internal_count.empty()) {
          int const data_count = lgb_tree.internal_count[old_id];
          TREELITE_CHECK_GE(data_count, 0);
          tree.SetDataCount(new_id, static_cast<size_t>(data_count));
        }
